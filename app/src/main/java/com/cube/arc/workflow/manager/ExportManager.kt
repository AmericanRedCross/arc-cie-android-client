...
import com.cube.arc.common.model.ApiError
...
object ExportManager {
        ...
        DirectoryManager.directories.forEach { directory ->
            ...
            directory.directories.forEach { step ->
                step.directories.forEach { substep ->
                    ...
                    rows[0][columns[0]] = ((step.metadata?.get("hierarchy") as String?
                            ?: "${step.order}") + " " + step.title).escapeCsv()
                    rows[0][columns[1]] = ((substep.metadata?.get("hierarchy") as String?
                            ?: "${substep.order}") + " " + substep.title).escapeCsv()
    ...
    fun download(file: FileDescriptor, path: File, progress: (percent: Int) -> Unit,
...
//					.addHeader("Last-Modified", lastModified)
                ...
                try {
                    ...
                    if (!response.isSuccessful && error != null) {
                        ...
                        error.invoke(response.code(), apiError.message)
                    } else {
