...
class ToolViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
    ...
    fun populate(root: Directory?, tool: Directory) {
        ...
        critical.apply {
            visibility = if ((tool.metadata?.getOrElse("critical_path", { false }) as Boolean
                            ?: false) || criticalPrefs.contains(tool.id.toString())) View.VISIBLE else View.GONE
        ...
        tool.attachments.getOrNull(0)?.let {
            ...
            toolDescription.visibility = if (it.description?.isEmpty()
                            ?: true) View.GONE else View.VISIBLE
        }

        toolIcon.setImageResource(tool.attachments.getOrNull(0)?.mimeIcon()
                ?: R.drawable.ic_mime_misc)
