...
object AnalyticsHelper
{
	...
	fun userCollapsesDirectory(directory: Directory) = sendEvent("%s %s".format(directory.metadata?.getOrDefault("hierarchy", null) ?: "", directory.title), "Collapsed")
	fun userTapsDirectoryRoadmap(directory: Directory) = sendEvent("%s %s".format(directory.metadata?.getOrDefault("hierarchy", null) ?: "", directory.title), "View roadmap")
	fun userChecksDirectoryCheckbox(directory: Directory) = sendEvent("%s %s".format(directory.metadata?.getOrDefault("hierarchy", null) ?: "", directory.title), "Checked")
	fun userUnchecksDirectoryCheckbox(directory: Directory) = sendEvent("%s %s".format(directory.metadata?.getOrDefault("hierarchy", null) ?: "", directory.title), "Unchecked")
	fun userTapsAddNote(directory: Directory) = sendEvent("%s %s".format(directory.metadata?.getOrDefault("hierarchy", null) ?: "", directory.title), "Add note")
	fun userTapsEditNote(directory: Directory) = sendEvent("%s %s".format(directory.metadata?.getOrDefault("hierarchy", null) ?: "", directory.title), "Edit note")
